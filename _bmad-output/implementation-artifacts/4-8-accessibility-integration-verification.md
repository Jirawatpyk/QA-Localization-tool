...
- **PB-4: `unknown` AI errors skipped fallback chain.** Unknown SDK exceptions bail immediately without trying fallback models. **FIXED**: added `'unknown'` to `FALLBACK_ELIGIBLE` set.
- **PB-5: `checkDoubleSpaces` false positive when source also has double spaces.** L1 flagged target double spaces even when source had them too (intentional formatting). **FIXED**: skip if source also has double spaces.
- **PB-6: L2 prompt missing `repeated_word` + `capitalization` in "L1 already checks" list.** AI could flag duplicates of L1 findings. **FIXED**: added to prompt.
- **Deferred (TD):** TD-AI-005 (CAS guard race), TD-AI-006 (L3 filter + L2 partial failure), TD-AI-007 (glossary lowConfidence gap)
