...
48. **Story complexity gate: cross-AC interaction check** — นอกจากนับ AC ≤8 ต้องเช็ค cross-AC interactions ถ้า AC ใดต้อง coordinate กับ >3 ACs อื่น (เช่น optimistic UI × Realtime × audit × score recalc) → **ต้อง split story** ก่อน lock AC. AC count อย่างเดียวไม่พอวัด complexity. (Epic 4 Retro — Story 4.2 มี 8 ACs แต่ cross-cutting concerns สูง → 3 CR rounds, 5 TDs, 4 production bugs)
...
- **AC limit: ≤8 per story.** Story 3.4 (11 ACs, 3 CR rounds) proved large scope = more rounds. If >8 ACs → must split before locking AC. Exceeding requires explicit approval + split plan. (Epic 3 Retro A2)
