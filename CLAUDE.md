...
45. **Agent/CR fix → re-read AC before implement** — เมื่อ pre-CR agent หรือ CR reviewer แนะนำ fix, ต้อง **re-read AC ที่เกี่ยวข้องก่อน implement**. Agent/reviewer เก่ง detect problem แต่ suggested fix อาจขัด spec (เช่น agent แนะนำ "query wider" แต่ AC บอก "current file only"). ขั้นตอน: (1) อ่าน finding เข้าใจ problem (2) ค้น AC ที่เกี่ยวข้อง + quote wording (3) ถ้า suggestion ขัด AC → follow AC (4) ใส่ AC ref ใน code comment (e.g., `// AC3: "in current file"`). ใช้กับทุก phase: pre-CR scan, CR R1/R2/R3 findings, party-mode suggestions. ห้าม trust suggestion โดยไม่ cross-check. (Story 4.6 E2E Retro — CF-H1 agent fix ลบ file filter → auto-reject leak ข้าม files, AC3 บอกชัด "in current file")
