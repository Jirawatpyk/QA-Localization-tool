...
- `setState` inside `useEffect` (React Compiler `react-hooks/set-state-in-effect` error — use render-time adjustment pattern: `const [prev, setPrev] = useState(prop); if (prev !== prop) { setPrev(prop); setState(newVal) }`)
...
42. **`--no-verify` FORBIDDEN** — ห้าม commit ด้วย `--no-verify` เด็ดขาด. ถ้า pre-commit hook fail (TS error, lint error) → **แก้ root cause ก่อน commit**. TS error ใน unrelated file → แก้ TS error นั้นก่อน. ไม่มีข้อยกเว้น — `--no-verify` สร้าง fix-on-fix chain ที่ bypass CR + สะสม hidden bugs. (Story 4.2 E2E Retro — H1 initialDataRef bug เกิดจาก commit `94f9d03` ที่ใช้ `--no-verify`)
43. **E2E must PASS before story "done"** — ถ้า story มี E2E spec, ต้อง **run จริง** (`npx playwright test <spec>`) + ทุก test GREEN ก่อน mark done ใน sprint-status. `test.skip()` unskipped ≠ tested. "Suite-level skip guard" (e.g., `test.skip(!process.env.INNGEST_DEV_URL)`) ต้อง verify ด้วยการ set env + run. (Story 4.2 E2E Retro — 12 tests "unskipped" แต่ไม่เคย run → 11 issues พบภายหลัง)
44. **Cross-file data flow review** — เมื่อ function ในไฟล์ A สร้าง/แปลง state ที่ถูกใช้ในไฟล์ B (เช่น optimistic update → merge guard, store write → component read, hook output → effect sync) → ต้อง verify: (a) iteration order ตรงกัน (Map insertion order ≠ visual sort order), (b) ref/closure ไม่ stale เมื่อ prop เปลี่ยน, (c) optimistic value ถูก clear หลัง server confirm. Pre-CR `feature-dev:code-reviewer` agent ตรวจ cross-file flows. (Story 4.2 Systematic Review Retro — C1/H2/H3 ไม่เจอเพราะ agents scan ทีละไฟล์)
45. **Test fail + 2 fix attempts → stop, call debug-explorer** — If any test (E2E, unit, integration) fails and manual fix attempts ≥ 2 still don't pass → **stop guessing, invoke `debug-explorer` agent immediately**. Do NOT attempt fix #3 without agent analysis. Provide: (1) error message + error-context.md snapshot (2) test flow that fails (3) relevant source files (4) hypotheses tried and why they failed. The agent traces cross-file data flow and reads production code — finds stacked root causes that manual debugging misses. (Story 4.6 E2E Retro — 10+ manual attempts wasted ~2 hrs; debug-explorer found 3 stacked bugs in 1 invocation ~2 min)
46. **Agent/CR fix → re-read AC before implement** — When any agent or CR reviewer suggests a fix, **re-read the relevant AC before implementing**. Agents detect problems well but suggested fixes may widen scope beyond spec boundaries (e.g., "query wider" when AC says "current file only"). If suggestion contradicts AC → follow AC, not the suggestion. Add AC ref in code comment: `// AC3: "in current file"`. (Story 4.6 E2E Retro — agent fix removed file filter → auto-reject leaked across files; AC3 clearly states "in current file")
47. **Pipeline "fail loud" — ห้าม silent drop** — ทุก pipeline step ที่ produce findings ต้อง assert result ถ้า `findingCount === 0` จาก non-trivial input (segments > 10) → **raise alert / throw error** ไม่ใช่ log + continue. Silent success = invisible failure. Integration test ต้อง assert `findingCount > 0` ไม่ใช่แค่ "pipeline completed". (Epic 4 Retro — TD-AI-004 L2/L3 findings silently dropped 17 วัน, pipeline reported "completed" ทุกครั้ง)
48. **Story complexity gate: cross-AC interaction check** — ก่อน lock AC ต้องเช็ค cross-AC interactions ถ้า AC ใดต้อง coordinate กับ >3 ACs อื่น (เช่น optimistic UI × Realtime × audit × score recalc) → **ต้อง split story** ก่อน lock AC. AC count อย่างเดียวไม่พอวัด complexity — cross-cutting concerns คือตัววัดจริง. (Epic 4 Retro — Story 4.2 มี 8 ACs แต่ cross-cutting concerns สูง → 3 CR rounds, 5 TDs, 4 production bugs)
49. **Verification story = mandatory last story ทุก epic** — ทุก epic ต้องจบด้วย verification/integration story ที่ test ของจริงทั้ง epic (real AI calls, real DB, real Realtime, real E2E). ห้ามใช้ mock. ถ้า epic ไม่มี verification story → SM ต้องสร้างก่อนปิด epic. (Epic 4 Retro — Story 4.8 จับ TD-AI-004 CRITICAL bug ที่ซ่อนมา 17 วัน; ถ้า Epic 3 มี verification story จะจับได้เร็วกว่า)
50. **"น่าจะผ่าน" ≠ "ผ่านจริง" — ต้อง run tests ก่อน claim done** — ห้าม mark story/epic done โดยไม่ run test suite จริง (`npm run test:unit` + `npm run lint` + `npm run type-check`). "เมื่อวานผ่าน" ไม่นับ — code changes อาจ introduce regression. ต้อง run + เห็น GREEN ด้วยตา. (Epic 4 Retro — พิสูจน์สดๆ ว่ามี 3 failing tests ที่ไม่มีใครรู้)
...
- **#79** Cross-file review mandatory ทุก epic: ทุก epic จบต้อง run `feature-dev:code-reviewer` cross-file review **ทุก feature ที่ epic แตะ** (ไม่ใช่แค่ story-level). Story-level CR (#44) ดูแค่ files ที่ story แก้ — ไม่จับ bugs ที่เกิดจาก **สะสมข้าม stories** (stale data, race condition ข้าม features). Epic 1-4 ไม่เคยทำ full cross-file → สะสม 15 P0 bugs. (Codebase Review Sprint 2026-03-28)
...
- **CR round target: ≤2 per story.** If R3+ needed → review root cause. Pre-CR quality scan (3 agents) should prevent most findings from reaching CR. (Epic 2 Retro A3)
...
- **AI pipeline: real integration test MANDATORY** — ทุก story ที่เกี่ยวกับ AI pipeline (L2/L3) ต้องมี integration test ที่ call real AI API (ไม่ mock) + verify findings ถูก insert เข้า DB จริง. ห้ามแค่ check "pipeline completed" — ต้อง check "findings count > 0". Mock-only unit tests ไม่จับ format mismatch ระหว่าง prompt → AI response → parser. Silent drop (logger.warn + continue) ห้ามใช้กับ core feature — ต้อง fail loud หรือมี metric counter. (Story 4.8 Retro — TD-AI-004 L2/L3 bracket bug ซ่อน 17 วันเพราะไม่มี real AI test)
