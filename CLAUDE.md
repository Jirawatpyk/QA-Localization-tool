...
46. **Agent/CR fix → re-read AC before implement** — When a pre-CR agent, CR reviewer, or party-mode agent suggests a fix, **re-read the relevant AC before implementing**. Agents detect problems well but suggested fixes may contradict spec scope boundaries. Steps: (1) read finding, understand the problem (2) find relevant AC, quote exact wording (3) verify fix doesn't widen query scope or data flow beyond AC boundaries (4) if suggestion contradicts AC → follow AC, not the suggestion (5) add AC reference in code comment (e.g., `// AC3: "in current file"`). Applies to all phases: pre-CR scan, CR R1/R2/R3, party-mode. (Story 4.6 E2E Retro — agent fix removed file filter → auto-reject leaked across files; AC3 clearly states "in current file")
