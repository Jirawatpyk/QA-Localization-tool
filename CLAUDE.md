...
45. **E2E fail + 2 fix attempts → stop, call debug-explorer** — If an E2E (or complex unit) test fails and manual fix attempts ≥ 2 still don't pass → **stop guessing immediately, invoke `debug-explorer` agent** with: (1) error message + error-context.md snapshot (2) test flow that fails (3) relevant source files (4) hypotheses already tried. The agent traces cross-file data flow and reads production code to find root cause faster than manual guess-and-fix loops. (Story 4.6 E2E Retro — 10+ manual fix attempts vs debug-explorer found 3 stacked root causes in 1 invocation)
46. **Agent/CR fix → re-read AC before implement** — When a pre-CR agent or CR reviewer suggests a fix, **re-read the relevant AC before implementing**. Agents/reviewers are good at detecting problems but their suggested fix may contradict the spec (e.g., agent suggests "query wider" but AC says "current file only"). Steps: (1) read finding, understand the problem (2) find relevant AC, quote exact wording (3) if suggestion contradicts AC → follow AC, not the suggestion (4) add AC reference in code comment (e.g., `// AC3: "in current file"`). Applies to all phases: pre-CR scan, CR R1/R2/R3, party-mode suggestions. Never trust suggestions without cross-checking AC. (Story 4.6 E2E Retro — CF-H1 agent fix removed file filter → auto-reject leaked across files, AC3 clearly states "in current file")
