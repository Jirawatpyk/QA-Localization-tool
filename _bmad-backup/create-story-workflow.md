...
<output>⚠️ COMPLEXITY GATE TRIGGERED — Story exceeds complexity threshold. - AC count: {{ac_count}} (limit: 8) - Max cross-AC interactions: {{max_interactions}} (limit: 3)
...
<action>From the story file, extract: - ALL DB table names referenced (e.g., findings, review_actions, feedback_events) - ALL type/interface names defined (e.g., FindingSnapshot, UndoEntry) - ALL files listed in "Files to modify" and "Existing Code to Extend" - ALL column/field names used in type definitions or server action schemas - ALL function signatures assumed (return types, parameters)
</action>
...
For each claim in the story, mark as: - ✓ VERIFIED — matches actual code - ✗ MISMATCH — story says X, code says Y → FIX in story - ⚠ MISSING — story assumes something that doesn't exist yet → note as "needs migration" or "needs creation"
</action>
