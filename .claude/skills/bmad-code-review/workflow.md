...
<action>Launch FOUR sub-agents IN PARALLEL using the Task tool to scan all files in the comprehensive review list: 1. code-quality-analyzer (subagent_type="code-quality-analyzer") — code quality, patterns, maintainability (single-file focus) 2. testing-qa-expert (subagent_type="testing-qa-expert") — test coverage gaps, missing test scenarios, test quality 3. feature-dev:code-reviewer (subagent_type="feature-dev:code-reviewer") — CROSS-FILE Data Flow Reviewer (Guardrail #44).
