...
<action>Launch FOUR sub-agents IN PARALLEL on all files in comprehensive review list: 1. code-quality-analyzer — code quality, patterns, maintainability (single-file) 2. testing-qa-expert — test coverage gaps, missing test scenarios 3. feature-dev:code-reviewer — CROSS-FILE data flow (Guardrail #44):
Phase 1: List ALL cross-file producer→consumer pairs
Phase 2: Verify per pair: (a) contract match, (b) lifecycle, (c) staleness,
(d) timing/race, (e) error/edge states. P0/P1 only, skip P2+.
DO NOT report single-file issues (= agent #1's job) 4. bmad-review-edge-case-hunter — boundary conditions: walk every branching path,
