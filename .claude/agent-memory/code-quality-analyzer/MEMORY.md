...
### Story 2.2 Parser — Idempotency & Race Condition Pattern

- `parseFile.action.ts` has NO idempotency guard on file status before parse
- No `WHERE status = 'uploaded'` in UPDATE to 'parsing' — concurrent calls duplicate segments
- No unique constraint on `segments(file_id, segment_number)` — DB allows duplicates
- Batch inserts NOT in a transaction — partial failure leaves orphaned segments
- **Pattern to check in all future actions:** Any status-transition action MUST use optimistic locking

### Story 2.2 Parser — NFKC Normalization Missing

- CLAUDE.md requires "NFKC normalization before text comparison"
- Parser stores raw text from XML without `.normalize('NFKC')` — affects downstream glossary matching and L1 rules
- Must normalize at parse time (before DB insert) since all downstream consumers assume normalized text
...
### Story 2.2 Parser — Audit Log Error Handling (RECURRING)

- `markFileFailed()` in `parseFile.action.ts` has no try-catch — same issue as Story 2.1 Round 3
- This is now confirmed as a cross-story recurring anti-pattern: writeAuditLog on error paths MUST be non-fatal
- Count: Story 2.1 (createBatch.action) + Story 2.2 (markFileFailed) = 2 occurrences
...
- Parser feature: `src/features/parser/{types,constants,inlineTagExtractor,wordCounter,sdlxliffParser}.ts` + `actions/`
