...
- Story 3.4 AI Resilience CR R2 (2026-03-07) — 0C + 0H + 3M + 4L. ALL R1 fixes confirmed clean. Violations: MEDIUMs: (1) `layersToRetry: ['L2'] as string[]` in retryFailedLayers.test.ts:136 buildRetryEvent() — should be `as PipelineLayer[]` (Guardrail #3); (2) `status: 'calculated' as string` in processFile.story34.test.ts:33 + retryFailedLayers.test.ts:49 mock shape — widens domain union to bare string (Guardrail #3); (3) `res.json() as Array<{ status: SeedFileStatus }>` cast in pipeline-admin.ts:60 skips runtime validation at fetch boundary (Guardrail #3 — LOW risk in E2E context, acceptable as LOW if team prefers cast pattern). LOWs: (4) `createMockStep()` returns `ReturnType<typeof vi.fn>` instead of typed interface matching Inngest step API (processFile.story34.test.ts:161, retryFailedLayers.test.ts:148) — prefer `<T>(id: string, fn: () => Promise<T>) => Promise<T>`; (5) `data[0].id` access in seedAiPartialFile pipeline-admin.ts:208 lacks length guard before access (Guardrail #4); (6) `failedLayers: string[]` in processFile.ts:30 should be `PipelineLayer[]` since only 'L2'/'L3' are ever pushed (Guardrail #3). R1 VERIFIED CLEAN: `pipeline.retry-failed-layers` dot-notation correct in all 4 occurrences; `PRIMARY_MODELS: Record<PipelineLayer, string>` fixed; `RetryFailedLayersEventData.layersToRetry: PipelineLayer[]` fixed; `deriveLayersToRetry()` return `PipelineLayer[]` fixed; no `as any` on onFailure in any inngest file; try-catch at handler level (not inside step.run); all withTenant() present; generateText+Output.object pattern; no .optional()/.nullish() in AI schemas; logAIUsage every path; budget checked; fallback audited.
