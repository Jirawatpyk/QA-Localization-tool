...
<workflow>
  ...
  <step n="2" goal="Load project context, story information, and verify ATDD gate">
    ...
    <action>Extract {{story_id}} from story_key using dash format (e.g., story_key "2-7-batch-summary" → story_id "2-7")</action>
    ...
    <check if="ATDD checklist file does NOT exist">
      <output>🚫 **ATDD GATE FAILED** — No acceptance tests found for Story {{story_id}}
      ...
      <action>HALT: "ATDD checklist missing — run TEA ATDD workflow first"</action>
    </check>
  ...
  <step n="9" goal="Pre-CR quality scan with sub-agents">
    ...
    <action>Launch FOUR sub-agents IN PARALLEL using the Task tool to scan all files in the story File List:
      ...
      4. feature-dev:code-reviewer (subagent_type="feature-dev:code-reviewer") — CROSS-FILE data flow review (Guardrail #44).
         Scope: ONLY state/data that crosses file boundaries (single-file quality = code-quality-analyzer).
         Method: For each changed file, identify what it EXPORTS (state, callbacks, types) and what it IMPORTS.
         Then trace each cross-file flow end-to-end and ask:
         - Does the data arrive at the consumer in the shape/order the consumer expects?
         - Is the data's lifecycle complete? (created → used → cleaned up across both files)
         - What happens when the producer is in a transitional state? (loading, unmounted, stale, revalidated)
         - Could timing between producer update and consumer read cause a bug? (race condition, stale closure, batched render)
