...
<activation critical="MANDATORY">
  ...
  <step n="11">PRE-CR QUALITY SCAN — After ALL tasks/subtasks are complete and tests pass:
      0. Run lint + type-check FIRST — fix any errors before launching agents (agents should scan compilable code)
      1. Launch FOUR sub-agents IN PARALLEL using the Task tool:
         ...
         - feature-dev:code-reviewer (subagent_type="feature-dev:code-reviewer") — CROSS-FILE Data Flow Reviewer (Guardrail #44). INITIAL SCOPE: If "Cross-file Pairs" section exists in story Dev Notes (from Step 2 codebase verification), use those pairs as starting scope — verify them FIRST, then discover additional pairs from code. Scope: ONLY state/data crossing file boundaries (single-file = code-quality-analyzer). Phase 1 — DISCOVERY (mandatory): List ALL cross-file pairs as [Producer] → [Consumer]: [what flows]. If 0 pairs → state "No cross-file data flows detected" and terminate. Phase 2 — VERIFICATION per pair: (a) contract match, (b) lifecycle completeness, (c) staleness, (d) timing/race, (e) error/edge states. RISK FILTER: P0 (data corruption, wrong output, silent data loss) + P1 (race, stale state, unhandled error propagation). Skip P2+. OUTPUT: 1) pair list 2) findings per pair with severity + evidence (file:line). DO NOT report single-file issues (= code-quality-analyzer's job)
