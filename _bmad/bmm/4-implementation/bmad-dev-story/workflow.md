...
<workflow>
  ...
  <step n="9" goal="Pre-CR quality scan with sub-agents">
    ...
    <!-- lint + type-check FIRST — agents should scan compilable code -->
    ...
    <action>Launch FOUR sub-agents IN PARALLEL on all files in story File List:
      ...
      4. feature-dev:code-reviewer (subagent_type="feature-dev:code-reviewer") — CROSS-FILE Data Flow Reviewer.
         Scope: ONLY state/data that crosses file boundaries (single-file = agent #3).
         INITIAL SCOPE: If "Cross-file Pairs" section exists in story Dev Notes (from Step 2),
           use those pairs as starting scope — verify them FIRST, then discover additional pairs.
         Phase 1 — DISCOVERY: List ALL cross-file pairs as [Producer] → [Consumer]: [what flows].
         Phase 2 — VERIFICATION per pair: (a) contract match, (b) lifecycle, (c) staleness, (d) timing/race, (e) error states.
         RISK FILTER: P0 + P1 only. Skip P2+.
         OUTPUT: pair list + findings with severity + evidence (file:line).
    </action>
